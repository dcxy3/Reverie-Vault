...
export function MusicPlayer({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      <audio ref={cloudAudioRef} onTimeUpdate={(event) => { const currentTime = event.currentTarget.currentTime; setProgress((current) => ({ ...current, netease: { ...current.netease, currentTime } })); }} onDurationChange={(event) => { const duration = event.currentTarget.duration || 0; setProgress((current) => ({ ...current, netease: { ...current.netease, duration } })); }} onPlay={() => { setPlayingSource("netease"); setPlaying(true); }} onPause={() => { if (playingSource === "netease") setPlaying(false); }} onEnded={() => adjacent(1, "netease")} onError={() => setError("网易云音频无法播放，请检查网络后重试")} />
      <audio ref={localAudioRef} onTimeUpdate={(event) => { const currentTime = event.currentTarget.currentTime; setProgress((current) => ({ ...current, local: { ...current.local, currentTime } })); }} onDurationChange={(event) => { const duration = event.currentTarget.duration || 0; setProgress((current) => ({ ...current, local: { ...current.local, duration } })); }} onPlay={() => { setPlayingSource("local"); setPlaying(true); }} onPause={() => { if (playingSource === "local") setPlaying(false); }} onEnded={() => adjacent(1, "local")} onError={(event) => setError(`本地音频无法播放（媒体错误 ${event.currentTarget.error?.code || "未知"}），请确认格式受 Chromium 支持`)} />
    </section>
  );
}
